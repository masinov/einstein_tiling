...
| Sturmian lattice construction | `akiyama-hamada-ito-sturmian-2026` | theory notes `07`--`09`; no implementation | minimal colored S0 proof-draft; natural three-role quotient refuted; unmarked K1 blocked | The full local closure, not intended images, must decode to irrational corridors. Erasing macro addresses gives the source's `{S,M,L}` system with rational periodic slopes. Write the complete collar table and prove any future-equivalent merges before geometry. |
