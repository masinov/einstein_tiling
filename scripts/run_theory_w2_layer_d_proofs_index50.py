#!/usr/bin/env python
"""Produce independently checked DRAT cores for the six index-50 S3 kills."""
...
SEARCH = ROOT / "docs/notebook/assets/theory-w2-layer-d-s3-index50.json"
OUT = ROOT / "docs/notebook/assets/theory-w2-layer-d-proof-index50.json"
CERT_DIR = ROOT / "docs/notebook/assets/theory-w2-layer-d-proof-index50"
...
def main():
    ...
    if not search["scope"]["representative_scans_complete"]:
        raise AssertionError("index-50 orbit search is incomplete")
    selected = {
        tuple(row["hnf"]): min(row["killing_mapping_indices"])
        for row in search["finalist"]["by_hnf"]
        if row["killing_mapping_indices"]
    }
    if len(selected) != 6:
        raise AssertionError(f"expected six index-50 kills, got {len(selected)}")
    ...
    print(f"index-50 proof tasks: {len(tasks)}; jobs={args.jobs}", flush=True)
    results = []
    ...
    payload = {
        "kind": "theory-w2-layer-d-index50-independent-drat-certificates",
        "schema_version": 1,
        "date": "2026-07-17",
        "scope": {
            ...
            "negative_scope": (
                "only six of the 18 frontier HNFs are obstructed; this does not "
                "close the index-50 shell or extend the complete prefix"
            ),
        },
        "provenance": {
