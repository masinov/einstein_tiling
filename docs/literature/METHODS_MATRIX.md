...
| Sturmian lattice construction | `akiyama-hamada-ito-sturmian-2026` | theory notes `07`--`12`; no implementation | Minimal colored S0 and symbolic K1P close in proof draft. K2E is an exact-realization contract; N5 gives an arity-three obstruction; K2H/N6 show exactly four hidden phase states are necessary and sufficient symbolically. No geometric phase or monotile exists yet. | After review, analyze whether one homochiral carrier's finite pose action can realize the four local phase classes without an absolute origin. No shape search or experiment is authorized. |
