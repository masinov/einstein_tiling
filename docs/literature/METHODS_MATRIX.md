...
| Sturmian lattice construction | `akiyama-hamada-ito-sturmian-2026` | theory notes `07`--`14`, `58`--`83`; exact source fixtures and cold verifiers | One AHI Section 10.1 source presentation is reconstructed exactly; local finite-relation compilation is closed abstractly by K74A/K74R; several separable, carrier-local, ordinary-star and additive erasure families are refuted. No unmarked polygon, total decoder, or monotile result is established. | Enter through `83_ahi_branch_closure.md`. The AHI-specific carrier/zipper ladder is frozen. Reopen only for a source-independent one-support theorem, scoped unmarked undecidability, a complete all-tilings construction, or a source theorem that changes the global realization problem. |
