...
| T2.C1 | Turtle cannot tile HNF \((1,0,k)\) for any \(k\ge4\) | proof-draft; formula machine-checked for 4≤k≤100 | T2.C0; two exact thin placement profiles | `04_w2_cokernel.md`; legacy `finalist_thin_gf2_support` |
...
| T2.D7 | One distinct-tail V4 signature plus the six-kite collision orbit forces placement density at most 1/2 on every area-admissible `L <= 2 Lambda` | conjecture; exact maxima match on k=4,8,20 controls | affine V4 circuit hypergraph; seek translation-invariant discharging or rational dual certificate | `a4_v4_density.py`; notebook session 36 |
...
| T2.D | Torus holonomy obstruction from tiling groups | in progress; complete certified prefix through index 60 | seek an infinite packing-family theorem or return to W1/W3 recognizability; larger finite prefixes remain controls only | complementary S3/A4 exhaustion, twist-union DRAT proofs, saturation countermodels, and one-orbit packing lemma |
...
| T3.0Q | The D4 colored-component/A6-collar correspondence and phase normalization form an exact finite equivalence kernel | machine-verified partial theorem; full physical-context equality open | 17↔17 bijection on 310 controls; 17/17 boundary-owner round trips; two determinant-one maps and exact inverses; three generated level-pair round trips; bare SFT obstruction `3565→80` radius-two seeds | `spectre_equivalence_chain.py`; `theory-w3-spectre-d4-equivalence.json`; session 56 |
