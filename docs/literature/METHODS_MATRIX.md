...
| Sturmian lattice construction | `akiyama-hamada-ito-sturmian-2026` | theory notes `07`--`11`; no implementation | Minimal colored S0 and symbolic K1D close in proof draft; K1P selects an explicitly non-Cartesian parity code, so independent-side realization fails for that compiler. This is not a universal coding no-go. | Design K2G on paper: whether one fixed boundary can realize the selected joint contact code while excluding unintended contacts. No search or experiment is authorized; do not claim finite-`kappa` MLD, entropy, or a monotile. |
