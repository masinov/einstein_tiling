...
| Sturmian lattice construction | `akiyama-hamada-ito-sturmian-2026` | theory notes `07`--`09`; no implementation | conditional compiler/Q0 proof drafts; S0 instantiation withdrawn; natural three-role quotient refuted | Primary Table 1 gives `12S+6M+6L`, not the inferred `2S+L` optimized templates, and the one-support remark is Turtle-specific. Derive an exact common-support language for the actual templates before any address table or geometry. |
