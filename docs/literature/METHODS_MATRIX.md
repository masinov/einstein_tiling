...
| Sturmian lattice construction | `akiyama-hamada-ito-sturmian-2026` | theory notes `07`--`12`; no implementation | Minimal colored S0 and symbolic K1P close in proof draft. K2E is an exact-realization contract; N5 shows K1P parity is not unary/binary decomposable, so ordinary side and corner keys cannot realize it. This is not a universal monotile no-go. | Analyze a translation-equivariant auxiliary phase on paper. No shape search or experiment is authorized; do not claim finite-`kappa` MLD, entropy, or a monotile. |
