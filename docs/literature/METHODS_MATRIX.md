...
|---|---|---|---|---|
| Canonical polyform identity | `smkgs-hat-2024` | `substrate/`, `src/einstein/polykites/known_shapes.py`, `tests/test_hat.py`, `tests/test_turtle.py` | implemented for Hat/Turtle anchors | Implement full `Tile(a,b)` membership and provenance-bearing family records. |
...
| Heesch/corona search | `kaplan-heesch-2022`, `kaplan-heesch-sat-code`, `kaplan-8kites-2023` | `src/einstein/polykites/coronas.py`, `kaplan-8kite-crosswalk.json` | independent implementation; per-shape external reproduction through `n=8` | All 116 public PDF records map bijectively to repository keys with exact `H_c` agreement: 108 at one, five at two, two periodic-anisohedral controls and the Hat. Treat A2 as a benchmark/filter, not novel census data. |
| Isohedral SAT | `kaplan-isohedral-sat-2024` | `src/einstein/polykites/isohedral.py` | implemented | Complete n<=8 counts match Myers; retain as an early portable filter while A1 handles periodic tilings with multiple transitivity classes. |
| General aligned periodic quotient search | `smkgs-hat-2024` Appendix A supplies alignment bridge | `src/einstein/polykites/periodic_quotients.py` | implemented for bounded quotients | Preserve three-valued bounded verdicts; no finite negative is a general aperiodicity proof. |
| Abelian coloring / tile homology | `conway-lagarias-tiling-groups-1990`, `lidjan-baralic-flat-surface-homology-2021` | `src/einstein/periodicity/invariants.py`; T2.C0--T2.C5 | classical method; Turtle worked control | Keep the compact thin-family certificate as a verifier example. Do not claim method novelty or extend W2 quotient shells. |
| Large exact patches | proof caveats in `cheritat-spectre-clusters-2024` | `src/einstein/polykites/patches.py` | implemented as finite evidence | Add continuability certificates or explicitly label dead-end boundary conditions. |
| Diffraction/module fingerprint | `baake-et-al-diffraction-2025` | `src/einstein/analysis/diffraction.py` | calibrated heuristic | Compare Fourier modules, not only estimated rank/symmetry; never use A4 as proof. |
...
| General recognisability | `walton-recognisability-2026` | `src/einstein/tilings/substitution.py` | theorem crosswalk implemented; hypotheses unmet | Walton requires a compact Hausdorff expansive `L`-sub hull; in the return-discrete case strict injectivity is equivalent to already excluding every periodic tiling. Use it as a post-aperiodicity theorem/control, not circularly. |
| Golden Hex / Sturmian / Ammann bars | `akiyama-araki-turtle-2025`, `james-smith-rhombic-2024` | `src/einstein/tilings/sturmian/turtle.py`; Turtle A3 artifact | partial | Exact word recurrences and density algebra reproduce the published chirality target; encode forced GAB geometry and attempt blind symbolic-language recovery. |
| Sturmian lattice construction | `akiyama-hamada-ito-sturmian-2026` | integrated AHI source/carrier theory plus byte-preserved source notes `07`--`83`; exact fixtures and cold verifiers | One AHI Section 10.1 source presentation is reconstructed exactly; local finite-relation compilation is closed abstractly by K74A/K74R; several separable, carrier-local, ordinary-star and additive erasure families are refuted. No unmarked polygon, total decoder, or monotile result is established. | Enter through `docs/theory/sturmian/ahi_source_and_carriers.md` and `docs/theory/case_studies/ahi_realization.md`. The AHI-specific carrier/zipper ladder is frozen. Reopen only for a source-independent one-support theorem, scoped unmarked undecidability, a complete all-tilings construction, or a source theorem that changes the global realization problem. |
...
| Single-tile and small-support simulation | `greenfeld-tao-periodic-counterexample-2024`, `ollinger-fixed-polyominoes-2009`, `demaine-et-al-one-tile-2014`, `stade-two-tiling-2025`, `socolar-taylor-hexagonal-2011`, `akiyama-ammann-2012`, `lagae-kari-dutre-corner-2006`, `mampusti-whittaker-dendrite-2020`, `fletcher-atlas-2010` | K5C, theory notes 19--20 and 67--68; `reviews/K5C_SINGLE_TILE_SIMULATION.md`; `reviews/MARKED_STURMIAN_UNDECIDABILITY.md` | The marked one-connected-polygon Sturmian compiler problem is undecidable. Independent two-participant collar erasure is completely classified by biclique compatibility and fails uniformly on Stade's fixed relation. No general one-unmarked-polygon reduction follows. | Any next erasure must be explicitly nonseparable: third participant, multi-port context, carrier/verifier fusion, or a proved whole-plane redundancy in the marked relation. |
| T-junction / non-edge-to-edge contact complexes | `smkgs-chiral-2024`, `hellouin-lutfalla-vanier-geometric-2026`, `ollinger-fixed-polyominoes-2009`, `vereshchagin-matching-2026` | theory notes 21 and 82; `reviews/TJUNCTION_CONTACT_COMPLEX.md`; `reviews/FINITE_AUTOMATON_CONTACT_COMPILER.md` | rooted three-participant chains compile every finite automaton/relation; standard finite-state contact semantics, not method novelty | Local relation expressivity is closed. Do not build another zipper, delimiter, selector, or reset carrier. The unsolved step is one connected unmarked support forcing the complete topology and decoder in every tiling. |
