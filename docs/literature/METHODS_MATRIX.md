...
| Sturmian lattice construction | `akiyama-hamada-ito-sturmian-2026` | `docs/theory/07_stm1_sturmian_monotile_design.md`, `08_stm1_equal_support_compiler.md`; no implementation | minimal colored S0 proof-draft; unmarked K1 open | Equidistancing plus the `2M` split gives one triangular support and raw macro-address counts `18,18,2`; a finite collared atlas preserves minimal aperiodicity. K1 must encode its full refinement or prove a smaller quotient. Positive-entropy transport and full-isometry homochirality remain unproved. |
