...
| Sturmian lattice construction | `akiyama-hamada-ito-sturmian-2026` | theory notes `07`--`11`; no implementation | Minimal colored S0 and symbolic K1D close in proof draft; K1D distributes state over three coupled contact modes; role-only, independent-rail, constant-mode and independent-side reductions fail | Audit K2G: whether one fixed boundary can realize the joint contact code while excluding unintended contacts. No geometry is authorized yet; do not claim finite-`kappa` MLD, entropy, or a monotile. |
