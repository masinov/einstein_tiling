...
## D-0208 (2026-07-27) — Admit the 17-rhombus polygon as a local source compiler

ERR-017 supersedes K54S's role-only rejection.  Under the primary AHI rule,
SAB continuation is an exposed-endpoint condition, not exact tangent
equality.  All four minimum equalizers therefore realize the exact local
relation `large_A+2M <-> large_B+2M` on one connected unmarked 16-gon.

This is a finite local compiler only.  It is not promoted as a monotile until
whole-plane tilability and a total decoder for every tiling are proved.

## D-0209 (2026-07-27) — Make the source composition cone an admission gate

For any carrier-local fusion decoder, the target source occurrence vector
must lie in the nonnegative cone generated by its finite state-composition
vectors.  The AHI source requires
`#small_M/#large=6*(sqrt(2)-1)`.  A fixed `large+2M` carrier, and every
mixture using only the transcribed large-containing 17/49/51 kernels, has
maximum ratio two and is impossible.

No further carrier synthesis is admitted without listing its complete
composition vectors and passing this cone test first.

## D-0210 (2026-07-27) — Target the forced two-state P17 decoder

The fixed 17-rhombus support also has an endpoint-legal local all-singleton
state `Z=(0,17)`.  Together with `G=(1,2)` it passes the composition gate, but
any total decoder must force the exact irrational ratio
`frequency(Z)/frequency(G)=(6*sqrt(2)-8)/17`.

The constructive target is now the single explicit polygon `P17`, not a new
carrier family: prove it tiles, classify every tiling into the `G/Z` source
