...
| Sturmian lattice construction | `akiyama-hamada-ito-sturmian-2026` | `docs/theory/07_stm1_sturmian_monotile_design.md`; no implementation | audited theorem design; blocked before geometry | Minimal ST-M1 and the positive-entropy strengthening are separated. Derive the `kappa=infinity` equal-support colored source and a coupled three-state contact-star kernel; independent sofic rails cannot work. Full-isometry handling requires proved homochirality. |
